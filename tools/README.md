...
| [`e2e-ticket-15.mjs`](e2e-ticket-15.mjs) | 真 HTTP：**一个框** —— `/` 是产品、`/demo.html` 仍是三幕页（页签与旧入口都在、合体页的元素一个都没有）、`POST /api/drop` 的响应形状一个字没变（合体路由是加法）、**四种分发各走一遍**（碎片不给判断也不问模型、情绪句尾「吗」按输入类型算碎片、问事实→追溯并指出出处且**不塞判断**、问没说过的事→明说没找到、问自己→多条汇成答案）、**提问那句自己绝不作为自己的出处**、空投递 400、演示模式下三幕能从这个框跑出来 | `node tools/e2e-ticket-15.mjs` |

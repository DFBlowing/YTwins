---
name: code-review
description: "沿两个轴向审查自某个固定基点（commit、branch、tag 或 merge-base）以来的改动：Standards（代码是否遵循本仓库文档化的编码规范？）与 Spec（代码是否符合最初的 issue/spec 要求？）。两个审查由并行的 sub-agent 同时运行，并排汇报。当用户想审查某个 branch、某个 PR、进行中的改动，或提出 \"review since X\" 时使用。"
---
